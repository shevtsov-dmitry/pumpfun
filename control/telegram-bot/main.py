...
async def admin_panel(update: Update, context: CallbackContext) -> None:
    ...
    await update.callback_query.edit_message_text(
        text="Admin Panel", reply_markup=reply_markup
    )
...
async def admin_options(update: Update, context: CallbackContext) -> None:
    ...
    match query.data:
        ...
        case "change_url_twitter":
            await query.edit_message_text(
                "Please write new twitter url. Example: https://twitter.com/corvetteCar",
                reply_markup=reply_markup,
            )
        case "change_url_dex":
            await query.edit_message_text(
                "Please write new telegram url. Example: https://twitter.com/corvetteCar",
                reply_markup=reply_markup,
            )
        case "change_url_tground":
            await query.edit_message_text(
                "Please write new tensor url. Example: https://twitter.com/corvetteCar",
                reply_markup=reply_markup,
            )
...
async def options_change_urls(query):
    try:
        ...
        for website_name, _ in m.items():
            ...
            websites.append(
                [InlineKeyboardButton(website_name, callback_data=callback_data)]
            )
        ...
        await query.edit_message_text(
            text=text, reply_markup=InlineKeyboardMarkup(websites)
        )
    except requests.RequestException as e:
        await query.edit_message_text(
            "При попытке получить URL-адреса произошла ошибка",
