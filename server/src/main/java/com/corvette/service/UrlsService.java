...
public class UrlsService {
    ...
    public ResponseEntity<String> addUrl(String primarySiteDomain, String socialMediaName, String socialMediaLink) {
        primarySiteDomain = decode(primarySiteDomain);
        socialMediaName = decode(socialMediaName);
        socialMediaLink = decode(socialMediaLink);
        final MainWebsite mainWebsite = mainWebsiteRepo.findFirstByDomainName(primarySiteDomain);
        if (mainWebsite == null) {
            return new ResponseEntity<>("Can't find \"%s\" main website database%n".formatted(primarySiteDomain), HttpStatus.NOT_FOUND);
        }
        RedirectedUrlsUtils urlsUtils = new RedirectedUrlsUtils(mainWebsite, socialMediaName, socialMediaLink);
        ResponseEntity<String> socialMediaName1 = save(socialMediaName, socialMediaLink, mainWebsite);

        RedirectedUrl url = redirectedUrlRepo.findFirstByName(socialMediaName);
        if (url.getName() == null) {
            redirectedUrlRepo.save(new RedirectedUrl(mainWebsite, socialMediaName, socialMediaLink));
            return ResponseEntity.ok("saved website name: %s url: %s\n".formatted(socialMediaName, socialMediaLink));
        }
        return null;
        if (socialMediaName1 != null) return socialMediaName1;
    ...
    private class RedirectedUrlsUtils {
        ...
        public ResponseEntity<String> update() {
            try {
                ...
                return ResponseEntity.ok("updated website name: %s url: %s\n".formatted(socialMediaName, socialMediaLink));
    ...
    public ResponseEntity addMainWebsite(String name) {
        return mainWebsiteRepo.save(new MainWebsite(name)) != null ? new ResponseEntity<>(HttpStatus.OK) : new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    }
}
